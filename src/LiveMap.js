...
export default function LiveMap({ lat, lng, trail = [] }) {
  ...
  const mX = T + fX * T, mY = T + fY * T;

  return (
    <div style={{ borderRadius: 10, overflow: "hidden", border: "2px solid #334155" }}>
      <div style={{ position: "relative", width: T * 3, height: T * 3, maxWidth: "100%", margin: "0 auto", overflow: "hidden" }}>
        {tiles.map(({ dx, dy, x, y }) => (
